...
@Database(entities = [EqProfile::class, DeviceMapping::class], version = 7, exportSchema = false)
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        val MIGRATION_6_7 = object : Migration(6, 7) {
            override fun migrate(db: SupportSQLiteDatabase) {
                try {
                    db.execSQL("ALTER TABLE eq_profiles ADD COLUMN soundBoosterLimiterEnabled INTEGER NOT NULL DEFAULT 1")
                } catch (e: Exception) {
                    e.printStackTrace()
                }
            }
